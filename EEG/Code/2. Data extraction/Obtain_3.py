...
def f_waves(data_bi,start,end):
    ...
    #Obtain first and second derivative
    dxdt=np.zeros([len(data_bi),len(data_bi[0,start:end])-1])
    dx2dt=np.zeros([len(data_bi),len(data_bi[0,start:end])-1])
    for i in range(0,len(data_bi)-1):
        for j in range(0,end-start-1):
            dxdt[i,j]=data_bi[i,start+j+1]-data_bi[i,j+start]
            if j<0:
                dx2dt[i,j]=dxdt[i,j]-dxdt[i,j-1]
    A=np.var(data_bi[:,start:end])**2
    M=np.var(dxdt)/np.var(data_bi[:,start:end])
    C=np.sqrt((np.var(dx2dt)/np.var(dxdt))**2-(np.var(dxdt)/np.var(data_bi[:,start:end]))**2)
    print((np.var(dx2dt)/np.var(dxdt))**2-(np.var(dxdt)/np.var(data_bi[:,start:end]))**2)
    
    waves_avg=np.zeros(39)
